...
export default class ReportDetailPresenter {
  ...
  async notifyReportOwner(commentId) {
    try {
      ...
      if (!response.ok) {
        console.error('notifyReportOwner: resposne:', response);
        return;
      }
  ...
  async removeReport() {
    try {
      ...
      this.#view.removeReportSuccessfully('Success to remove from bookmark');
    } catch (error) {
      console.error('removeReport: error:', error);
      this.#view.removeReportFailed(error.message);
    }
  }
